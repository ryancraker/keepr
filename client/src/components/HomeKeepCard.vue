...
<template>
  <section class="keep-card container" :style="{ backgroundImage: `url(${keep.keep.img})` }">
    ...
    <div class="focus-modal" role="button" @click="focusKeep()" data-bs-target="#focusedKeepModal"
      data-bs-toggle="modal" :title="`Open '${keep.keep.name}' in a modal`">
        ...
        <div class="d-flex justify-content-between align-items-end keep-title p-1 position-relative">
          ...
          <RouterLink :to="{ name: 'Profile', params: { profileId: keep.keep.creatorId } }">
            <img :title="`Go to ${keep.keep.creator.name}'s page`" class="creator-pic" :src="keep.keep.creator.picture"
              :alt="keep.keep.creator.name" :class="{ 'd-none': route.path.includes('profile') }" />
          </RouterLink>
        </div>
      </div>
    </div>
